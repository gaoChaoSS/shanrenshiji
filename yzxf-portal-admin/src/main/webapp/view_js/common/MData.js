(function (angular, undefined) {
    ...
    window.app.register.controller(model + '_' + entity + '_Ctrl', function ($rootScope, $scope, $location, $http, $element, $compile, $templateCache) {
        ...
        $scope.$on('tree_left/itemClicked_after', function (event, obj) {
            ...
            if (obj.level == -1) {
                filter = {
                    includeKeys: 'name,title,desc,sortNo'
                };
            } else if (obj.level == 0) {
                filter = {
                    includeKeys: 'name,title,desc,tableType,sellerOwner,storeOwner,isSyncTable,sortNo'
                }
            } else {
                filter = {
                    includeKeys: null,
                    excludeKeys: 'modelName,entityName,sellerOwner,storeOwner,pid,_id,level,tableType'
                }
            }
            if (initGrid) {
                gridQuery();
