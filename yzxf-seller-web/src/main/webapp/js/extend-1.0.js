...
//页面处理数据时排它操作
    window.showLoading = function () {
        ...
        if (markLoading.size() == 0) {
            markLoading = $('<div id="markLoading" style="cursor:not-allowed"></div>');
...
(function () {
    ...
    function _malert(title,content, time, func) {
        if ($(document).find("#alertmsgDiv").length > 0) {
            $("#alertmsgDiv").remove();
            $("#alertbgDiv").remove();
        }
        ...
        msgObj.setAttribute("msgId", genUUID());
        ...
        msgObj.style.top = ($(window).height() - msgh ) / 2 + "px";
        msgObj.style.width = msgw + "px";
        //msgObj.style.height = msgh + "px";
        msgObj.style.textAlign = "center";
        msgObj.style.zIndex = "10001";
        msgObj.style.borderRadius = "10px";
        msgObj.style.animationFillMode = "forwards";
        msgObj.className += "alert-anim";
        document.body.appendChild(msgObj);
        // 提示信息标题
        var titleCon = document.createElement("div");
        titleCon.setAttribute("id", "alertmsgTitle");
        titleCon.setAttribute("align", "left");
        titleCon.style.margin = "0";
        titleCon.style.padding = "30px 20px 15px";
        titleCon.style.fontSize = "17px";
        titleCon.style.color = "#000";
        titleCon.style.textAlign = "center";
        titleCon.innerHTML = title;
        document.getElementById("alertmsgDiv").appendChild(titleCon);
        // 提示信息
        var txt = document.createElement("p");
        txt.setAttribute("id", "msgTxt");
        txt.style.margin = "0 0 20px 0";
        txt.style.color = "#888";
        txt.style.fontSize = "14px";
        txt.style.lineHeight = "20px";
        txt.style.padding = "0 20px";
        if(window.isEmpty(content)){
            content="";
        }
        txt.innerHTML = content;
        document.getElementById("alertmsgDiv").appendChild(txt);

        var cancelBtn = $("<button>知道了</button>").css({
            "color":"#00cc99",
            "width":"100%",
            "font-size":"17px",
            "background":"none",
        ...
        cancelBtn.click(function () {
            window.closewin(msgObj.getAttribute("msgId"));
        });
        $("#alertmsgDiv").append(cancelBtn);
    }
...
(function(){
    function _mconfirm(title,content, callback, cancelCallBack) {
        var msgw, msgh, bordercolor;
        msgw = 300;// 提示窗口的宽度
        msgh = 120;
        var sWidth, sHeight;
        ...
        var cancelBtn = $("<button>取消</button>").css(btn).css("color","#999");
        btn['border-left'] = '1px solid #e5e5e5';
        var confirmBtn = $("<button>确定</button>").css(btn);
        ...
        btnCon.append(cancelBtn).append(confirmBtn);
        $("#alertmsgDiv").append(btnCon);
    }

    function _closewin(msgId) {
        if (document.getElementById("alertmsgDiv")) {
            document.getElementById("alertmsgDiv").className = "alert-anim-close";
            setTimeout(function () {
                if(isEmpty(msgId) || msgId == document.getElementById("alertmsgDiv").getAttribute("msgId")) {
    ...
    window.closewin = _closewin;
    window.mconfirm = _mconfirm;
})();
...
// 时间相关
Date.prototype.addMilliseconds = function (d) {
    this.setMilliseconds(this.getMilliseconds() + d);
    return this;
};
