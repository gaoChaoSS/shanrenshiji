(function(angular, undefined) {
    ...
    window.app.register.controller('my_modifyPassword_Ctrl', function($rootScope, $scope, $location, $http, $element, $compile) {
        $scope.check = 'disabled';
        //完成按钮判断
        $scope.nextBtn=function(){
            $scope.check = 'disabled';
            if(!window.isEmpty($scope.oldPwd) && /^.{6,16}$/.test($scope.firstPwd)
